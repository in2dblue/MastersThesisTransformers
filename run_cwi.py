...
def main():
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        for mode in modes:
            result, predictions = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode=mode)
            # Save results
            ...
            # Save predictions
            output_test_predictions_file = os.path.join(args.output_dir, "{}_predictions.tsv".format(mode))
            with open(output_test_predictions_file, "w") as writer:
                with open(os.path.join(args.data_dir, "{}.tsv".format(mode)), "r") as f:
                    example_id = 0
                    for line in f:
